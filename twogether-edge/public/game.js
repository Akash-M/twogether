// Twogether — real-time spike client.
// Plain ES module, no bundler. Realtime via PartySocket; QR via the qrcode pkg (both from a CDN).
...
function connect(room) {
  roomId = room;
  socket = new PartySocket({ host: location.host, room });
  socket.addEventListener("message", onMessage);
  socket.addEventListener("open", () => {
    // (re)start ping when (re)connected
  });
  startPing();
}
...
function startPing() {
  setInterval(() => {
    if (socket && socket.readyState === 1) {
      const id = ++pingId;
      socket.send(JSON.stringify({ t: "ping", id, c: performance.now() }));
    }
  }, PING_MS);
}
...
function sendInput(force) {
  ...
  lastSentAt = now;
  lastSent = i;
  if (socket && socket.readyState === 1) socket.send(JSON.stringify({ t: "input", ...i }));
}
...
$("btn-again").onclick = () => {
  if (socket) socket.send(JSON.stringify({ t: "reset" }));
  won = false;
  show("play");
};
